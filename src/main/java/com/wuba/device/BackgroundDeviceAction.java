...
public class BackgroundDeviceAction extends Thread {
	private static Logger LOG = Logger.getLogger("BackgroundDeviceAction.class");
    ...
    public void run() {
        while (!isCancelled()) {
            if (mLogStartDelay > 0) {
            	LOG.debug(String.format("Sleep for %d before starting %s for %s.", mLogStartDelay, mDescriptor,
                        mSerialNumber));
                getRunUtil().sleep(mLogStartDelay);
            }
            LOG.debug(String.format("Starting %s for %s.", mDescriptor, mSerialNumber));
            try {
    ...
    private void recoverDevice(String exceptionType) {
    	LOG.debug(String.format("%s while running %s on %s. May see duplicated content in log.", exceptionType,
        ...
        try {
        	...
        	LOG.debug(String.format("Device %s not online", mSerialNumber));
        }
    }
