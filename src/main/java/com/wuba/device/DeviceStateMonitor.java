...
class DeviceStateMonitor implements IDeviceStateMonitor {
	private static Logger LOG = Logger.getLogger("DeviceStateMonitor.class");
    ...
    public boolean waitForDeviceShell(final long waitTime) {
    	LOG.info(String.format("Waiting %d ms for device %s shell to be responsive", waitTime,
                getSerialNumber()));
        ...
        while (System.currentTimeMillis() - startTime < waitTime) {
            ...
            try {
                ...
                LOG.info(String.format("%s failed: %s", cmd, e.getMessage()));
            } catch (TimeoutException e) {
                LOG.info(String.format("%s failed: timeout", cmd));
        ...
        LOG.warn(String.format("Device %s shell is unresponsive", getSerialNumber()));
        return false;
    }
    ...
    private boolean waitForBootComplete(final long waitTime) {
        LOG.info(String.format("Waiting %d ms for device %s boot complete", waitTime, getSerialNumber()));
        long startTime = System.currentTimeMillis();
        ...
        LOG.warn(String.format("Device %s did not boot after %d ms", getSerialNumber(), waitTime));
        return false;
    }
    ...
    private boolean waitForPmResponsive(final long waitTime) {
        LOG.info(String.format("Waiting %d ms for device %s package manager",
