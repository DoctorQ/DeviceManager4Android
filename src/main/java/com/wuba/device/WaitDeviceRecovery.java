 ...
 * be online and respond to simple commands.
 */
public class WaitDeviceRecovery implements IDeviceRecovery {
	private static Logger LOG = Logger.getLogger("WaitDeviceRecovery.class");
	...
	public void recoverDevice(IDeviceStateMonitor monitor,
		...
		if (monitor.getDeviceState().equals(TestDeviceState.FASTBOOT)) {
			...
			getRunUtil().runTimedCmd(20 * 1000, "fastboot", "-s",
	...
	protected void handleDeviceUnresponsive(IDevice device,
			IDeviceStateMonitor monitor) throws DeviceNotAvailableException {
		...
		if (monitor.waitForDeviceAvailable(mWaitTime) == null) {
			throw new DeviceUnresponsiveException(String.format(
					"Device %s is online but unresponsive",
					monitor.getSerialNumber()));
		}
	}
	 ...
	 *
	 * @param monitor
	 *            the {@link IDeviceStateMonitor}
	 * @param recoverTillOnline
	 *            if true this method should return if device is online, and not
	 *            check for responsiveness
	 * @throws DeviceNotAvailableException
	 */
	protected void handleDeviceNotAvailable(IDeviceStateMonitor monitor,
			boolean recoverTillOnline) throws DeviceNotAvailableException {
		throw new DeviceNotAvailableException(String.format(
				"Could not find device %s", monitor.getSerialNumber()));
	}
	...
	/**
	 * Handle condition where device is online, but should be in bootloader
	 * state.
	 * <p/>
	 * If this method
	 * 
	 ...
	 * @throws DeviceNotAvailableException
	 */
	protected void handleDeviceOnlineExpectedBootloader(
			final IDeviceStateMonitor monitor)
			throws DeviceNotAvailableException {
	 ...
	 * @throws DeviceNotAvailableException
	 */
	protected void handleDeviceBootloaderUnresponsive(
			IDeviceStateMonitor monitor) throws DeviceNotAvailableException {
		...
		// TODO: retry reboot
		getRunUtil().runTimedCmd(20 * 1000, "fastboot", "-s",
				monitor.getSerialNumber(), "reboot-bootloader");
	...
	protected void rebootDeviceIntoBootloader(IDevice device) {
		try {
			...
			Log.w(LOG_TAG,
					String.format("failed to reboot %s: %s",
							device.getSerialNumber(), e.getMessage()));
		} catch (TimeoutException e) {
			Log.w(LOG_TAG,
					String.format("failed to reboot %s: timeout",
	...
	/**
	 * Handle situation where device is not available when expected to be in
	 * bootloader.
	 *
	 * @param monitor
	 *            the {@link IDeviceStateMonitor}
	 * @throws DeviceNotAvailableException
	 */
	protected void handleDeviceBootloaderNotAvailable(
			final IDeviceStateMonitor monitor)
			throws DeviceNotAvailableException {
		throw new DeviceNotAvailableException(String.format(
				"Could not find device %s in bootloader",
				monitor.getSerialNumber()));
	}
